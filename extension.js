...
//dialog.add([_status.pileTop]);
 ...
        for(var i=0;i<cards.length;i+=(1)){var usecard=cards[i];if(usecard.name!='sha'||!target.hasSkill('diewulimitai_shale')){target.chooseUseTarget(usecard,true,'distance')};if(usecard.name=='sha'){target.addSkill('diewulimitai_shale');};};
 ...
        for(var i=0;i<cards.length;i+=(1)){var usecard=cards[i];if(usecard.name!='sha'||!target.hasSkill('diewulimitai_shale')){target.chooseUseTarget(usecard,true,'distance');}};
...
   event.choiceList.push('+'+f+'→'+(f+1)+'手牌上限，能于更远距离给其他角色递牌给杀，<br>一般是用来降低摸牌门槛。');};
 ...
        if(event.targets.length<2&&!get.attitude(player,event.target)>=0) return false;
    ...
    "step 0"
    player.update();game.log(trigger.getParent().triggeredTargets3,trigger.getParent().triggeredTargets2,trigger.getParent().triggeredTargets1,trigger.getParent().excluded);
    player.chooseTarget(get.prompt('防空保护对象'),('X为选择的角色数，当一名角色使用的锦囊牌指定了至少两名角色为目标时，你可以消耗X的手牌，令此牌对X名角色无效。'),
       [1,Math.min(trigger.targets.length,Math.floor(player.countCards('he')))],function(card,player,target){
 ...
   // }
    ...
    "step 1"
    if(result.bool){ 
        var num=result.targets,target=result.targets.length;event.target=result.targets//event的赋值不需要var 可以一直使用的事件结束
   game.log(event.target);
    var discard=player.countCards('he');
    var next=player.chooseToDiscard(get.prompt('防空弃牌事件'),target,'弃置两倍的手牌并让锦囊牌对目标角色无效');
    next.setHiddenSkill('fangkoganning');//还要隐藏还行skillAnimation:true,    animationColor:"wood",
    next.ai=function(card){
        if(discard){if(ui.selected.cards.length>=Math.max(player.countCards('h')/3,1)) return -1;
 ...
        game.delay();
        if(event.target!=undefined){for(var i=0;i<trigger.targets.length;i+=(1)){if( event.target.contains(trigger.targets[i])){ trigger.getParent().excluded.add(trigger.targets[i]);trigger.targets[i].addSkill('fangkoganning_aibiexuan');game.log('取消卡牌目标',trigger.targets[i],'编号',i)}}};
        player.logSkill('fangkoganning',event.target);
        event.finish;
    }
