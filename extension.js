game.import("extension",function(lib,game,ui,get,ai,_status){return {name:"舰R牌将",content:function (config,pack){
 ...
        if(target==player.previous){
        ...
        if(evt&&evt.name=='phaseUse'){player.turnOver();target.insertPhase();event.finish();}};
...
if(!player.hasSkill('rendeonly2')){player.addSkill('diewulimitai');};
	...
	if(jianrjinji.enable){
		...
		character:{
            liekexingdun:["female","wu",4,["hangmucv","yuanhang","kanpolimitai","roundonefire","xianjinld","danzong","jianzaochuan","wulidebuff","qianghuazhuang"],["zhu","des:血量中等的航母，温柔，体贴，过渡期追着大船打的航母。"]],
            chicheng:["female","wu",4,["hangmucv","yuanhang","kanpolimitai","roundonefire","xianjinld","danzong","jianzaochuan","wulidebuff","qianghuazhuang"],["des:大佬友情放出精美壁纸，坚定与自信的姿态"]],
            bisimai:["female","shu",4,["yuanhang","zhanliebb","huokongld","zhuangjiafh","fupaozu","danzong","huijiahuihe","jianzaochuan","wulidebuff","qianghuazhuang"],["zhu","des:更多刮痧，更多力量，更多护甲，更高血量。"]],
            misuli:["female","shu",4,["yuanhang","zhanliebb","huokongld","zhuangjiafh","fupaozu","danzong","huijiahuihe","jianzaochuan","wulidebuff","qianghuazhuang"],["des:用精巧的手枪去质疑，用绝对的火力回击对手。"]],
            lizhan:["female","shu",4,["zhongxunca","yuanhang","zhuangjiafh","fupaozu","danzong","huijiahuihe","huokongld","jianzaochuan","wulidebuff","qianghuazhuang"],["des:这是个依赖科技的舰船，有着科幻的舰装，与兼备温柔体贴与意气风发的表现。"]],
            
            kunxi:["female","shu",4,["zhongxunca","yuanhang","zhuangjiafh","fupaozu","danzong","huijiahuihe","huokongld","jianzaochuan","wulidebuff","qianghuazhuang"],["des:画师优秀的功底让这名角色美而可爱，这是出色的角色塑造。"]],
            ougengqi:["female","shu",4,["zhongxunca","yuanhang","zhuangjiafh","fupaozu","danzong","huijiahuihe","huokongld","jianzaochuan","wulidebuff","qianghuazhuang"],["des:励志偶像，与标志性舰装，可惜没有适合的图与技能用于无名杀"]],
            yixian:["female","wei",3,["yuanhang","qingxuncl","danzong","fangkong2","huibi","jianzaochuan","wulidebuff","qianghuazhuang"],["des:经典美术设计的款式，意气风发，威猛先生"]],
            haiwangxing:["female","wei",3,["yuanhang","qingxuncl","danzong","fangkong2","huibi","jianzaochuan","wulidebuff","qianghuazhuang"],["des:阻敌计谋表现优秀，这是先发制敌的优势所在，"]],
            rending:["female","wei",3,["yuanhang","qingxuncl","danzong","fangkong2","huibi","jianzaochuan","wulidebuff","qianghuazhuang"],["des:手持竹伞的轻巡，辅助队友，防御攻击。"]],
            "z31":["female","wei",3,["yuanhang","quzhudd","huibi","zhiyangai","rendeonly2","danzong","jianzaochuan","wulidebuff","qianghuazhuang"],["des:婚纱与轻纱是多数人的美梦,与绿草平原，与绿水青山"]],
            xuefeng:["female","wei",3,["yuanhang","quzhudd","huibi","zhiyangai","rendeonly2","danzong","jianzaochuan","wulidebuff","qianghuazhuang"],["des:幸运的驱逐舰，多位画师、花了大款的大佬亲情奉献。"]],
            kangfusi:["female","wei",3,["yuanhang","quzhudd","huibi","zhiyangai","rendeonly2","danzong","jianzaochuan","wulidebuff","qianghuazhuang"],["des:水手服欸,优秀的构图，不过图少改造晚。"]],
            "u1405":["female","shu",1,["yuanhang","qianting","danzong","touxichuan","baiyin_skill","chongsheng","rendeonly2","roundonefire","jianzaochuan","wulidebuff","qianghuazhuang"],["des:无需隐匿的偷袭大师，马上就让对手的后勤捉襟见肘。"]],
            jingjishen:["female","wu",3,["junfu","yuanhang","jianzaochuan","reyaowu","chouce","wulidebuff","qianghuazhuang"],["des:需要武器支援，伙计倒下了。"]],
            changchun:["female","wu",3,["daoqu","tianyi","yuanhang","jianzaochuan","wulidebuff","qianghuazhuang"],["des:尚处于正能量之时。"]],
        },
        skill:{
            tiaozhanzhuanbei:{
                trigger:{
...
if(get.position(card)=='e'&&get.mode()=='boss'&&['equip1','equip5','equip6'].contains(get.subtype(card))) return false;},},
 ...
 return get.name(evt.card,'sha')=='sha'; }).length,danzong=player.getAllHistory('useSkill',function(evt){return evt.skill=="danzong";}).length;var e=Math.random(),f=0.2;if(player.hasSkill('quzhudd'))var f=0.15;if(player.hasSkill('qingxuncl'))var f=0.2;if(player.hasSkill('zhongxunca'))var f=0.25;
 return event.card.name=='sha'&&!event.card.nature&&!player.hasSkill('danzong_damage')&&e<f||chusha>danzong*6+6;
    },
...
for(var i=0;i<trigger.num;i+=(1)){if(trigger.num>0&&player.countMark('kaishimopao_draw')){trigger.num-=(1);player.removeMark('kaishimopao_draw',1)}}
 ...
        },
   ...
   if(event.type=='dying'){if(player!=event.dying) return false; return player.countCards('hejs')>=4;}
else if(event.parent.name=='phaseUse'&&(a)>0){return (player.countCards('hejs')>=3)&&a;    }return false;
     },
                selectCard:function(event,player){var event=_status.event;if(event.type=='dying')return 4;return 3;},
                position:"hejs",
 ...
        if(player.hp<=0&&(huifu<(-player.hp+1)||!player.hasMark('jianzaochuan')))  return 15-get.value(card);
                ...
                prompt:"你可以弃置三张同花色的牌，提升一点血量上限，再回复2点体力。重复建造只会回一点血，濒死建造需弃置四张牌",
                content:function(){//，且只会回一点血并随机弃置一张牌。
player.recover();player.addMark('jianzaochuan');
if(event.trigger.name=='dying'){player.discard(player.getCards('h').randomGet());event.finish()};
if(player.countMark('jianzaochuan')<2){player.gainMaxHp(1);player.recover();};
},
                ai:{
                    save:true,
                    expose:0,
...
filter:function(event,player){
 ...
        return !player.countCards('h','sha')||!player.canUse('sha',player);
        ...
        return false;
                ...
                prompt:"你可以弃置最多五张相同花色的牌，失去n-2点体力,然后对你选择的目标视为使用n-1张决斗，n为你弃置的牌数。<br>第3、4张决斗将造成属性伤害。",
                content:function(){
 ...
        player.chooseTarget(get.prompt2('选择攻击目标'),function(card,player,target){
    ...
    if(target.hasSkill('bagua_skill')|target.hasSkill('re_bagua_skill')){ att*=0.5};
        ...
        if(result.bool){
            ...
            event.target=target;
if(event.num>1) player.useCard({name:'juedou'},event.target);
if(event.num>2) player.useCard({name:'juedou'},event.target);
if(event.num>3) player.useCard({name:'juedou',nature:'thunder',isCard:false},event.target);
if(event.num>4) player.useCard({name:'juedou',nature:'fire',isCard:false},event.target);;
if(event.num>1) player.damage(event.num-2);}else event.finish();
 ...
        var next=player.chooseToDiscard('hejs',{color:'red'},[1,3],get.prompt('叠甲'),('你拥有护甲时，会减少等同于护甲值的手牌上限。<br>每当你结算完回复的血量，或受到的伤害时,<br>你可以弃置至多三张红色牌，获得X点护甲。（X为弃牌数）<br>若此时没有红色牌且护甲值没有发生变化，<br>你在伤害结算后获得1点护甲，而无需弃牌。'));
 ...
        player.chooseTarget(get.prompt2('在离开战斗前，你还可以令队友、关注的对象摸2*x-1张牌，或许会有转机出现(x为你体力上限,保底给目标3张)<br>如果目标是伤害来源则摸x张牌，保底摸一张牌'),function(card,player,target){
 ...
            target.draw(Math.max(3,player.maxHp*2-1));
        }
        else event.finish();
    },
                        sub:true,
 ...
        player.draw(1);if(player.maxHp>2){player.loseMaxHp(1);player.draw();player.logSkill('kurou',player);}else game.playAudio('..','extension','舰R牌将/audio','bingsimosanpai');if(player.maxHp>5){player.loseMaxHp(1);player.draw();game.log('血量上限好高啊，额外来一次扣血摸牌吧',player);}
 ...
            return tishi;  },
            ...
            qianting:{
mod:{
attackFrom:function(from,to,distance){var a=0;if(from.hasSkill('qianting')){var a=a+1};if(from.hasSkill('quzhudd')&&to.hasSkill('qianting')){var a=a-1};return distance=(distance+a)},
...
if(range[1]==-1) return;var a=game.countPlayer(function(current){return get.attitude(player,current)<0});
if(get.type(card)=='trick'){var info=get.info(card);
if(info.multitarget) return false;
range[1]+=Math.min(player.countMark('jinengup'),a-1);};},},
                audio:"retuxi",
                priority:2,
                trigger:{
                ...
                preHidden:true,
                filter:function(event,player){
        return event.num>1&&!event.numFixed&&game.hasPlayer(function(target){
            return target.countCards('h')>0&&player!=target;
        });
    },
                content:function (){
        "step 0"
        var num=get.copy(trigger.num)-1,wa=0;if(player.countMark('jinengup')>1){num+=(1)};;if(player.countMark('jinengup')>0){wa+=(1)};
       if(num<=0)event.finish();if(get.mode()=='guozhan'&&num>2) num=2;
        player.chooseTarget(get.prompt('new_retuxi'),'选择至多'+get.translation(num)+'名角色，你少摸目标数+'+(1-wa)+'张牌,然后<br>你对目标使用【顺手牵羊】（目标手牌数与装备数大于你时），否则<br>你对目标使用【过河拆桥】。<br>(需要摸牌次数>1);强化技能后：1级:少摸的牌-1；2级:可选择目标+1。',[1,num],function(card,player,target){
return target.countCards('he')>0&&player!=target;},function(target){
var att=get.attitude(_status.event.player,target);
if(target.hasSkill('tuntian')) return att/10;
if(target.countCards('he')>=player.countCards('he')+1) return 1-att*1.3;
 if(target.countCards('he')<player.countCards('he')+1){
if(player.countMark('jinengup')<0)  return -1;
if(target.getCards('e'))return 1-att*1.1;}  return 1-att;
        });
        "step 1"
        if(result.bool){
            result.targets.sortBySeat();
            player.logSkill('new_retuxi',result.targets);
            for(var i=0;i<result.targets.length;i+=(1)){var target=result.targets[i];if(target.countCards('he')>=player.countCards('he')+1){player.useCard({name:'shunshou'},target)}else player.useCard({name:'guohe'},target);};
            if(player.countMark('jinengup')>0){var wa=1};
            trigger.num-=Math.min(result.targets.length+1-wa,trigger.num)
        }
        else{
            event.finish();
        }
        "step 2"
        if(trigger.num<=0) game.delay();game.log(Math.random());
    },
                ai:{
                    threaten:1.6,
                    expose:0.2,
...
　　if((e1.name=='zhuge')||(e1.name=='rewrite_zhuge')) return 1.1;};
 ...
            return ('使用无懈的次数');
        ...
        player.insertPhase();
            ...
            touxichuan:{
                ...
                usable:2,
                position:"hejs",
                prompt:"将♦牌当做火攻，♥牌当做乐不思蜀，♣牌当作过河拆桥，♠牌当做兵粮寸断使用<r>火攻只需一张牌，其他视为牌需要一张装备牌或者两张基本牌。<br>锦囊牌可以对距离你为2以内的角色使用。",
                filter:function(event,player){if(event.parent.name=='phaseUse')return player.countCards('hs')>0;
    },
                viewAs:function(cards,player){
        var name=false;
        switch(get.suit(cards[0],player)){
            case 'club':name='guohe';break;
            case 'diamond':name='huogong';break;
            case 'spade':name='bingliang';break;
            case 'heart':name='lebu';break;
        }
 ...
        var player=_status.event.player,num=0;num+=(player.countMark('e'));if(ui.selected.cards.length&&get.type(ui.selected.cards[0],'equip')=='equip'){num+=(1)};if(ui.selected.cards.length&&get.suit(ui.selected.cards[0],player)=='diamond'){num+=(1)};
        return [Math.max(2-num,1),Math.max(2-num,1)];
    },
                ...
                content:function(){
            player.logSkill('qixi',target);game.log('要进水弃牌了');
...
   event.choiceList.push(event.jieshao[5]);};
 ...
        var next=player.chooseToDiscard('令本次攻击命中对手,<br>一级，你弃置一张牌，对面摸一张牌；二级：你弃置一张牌。',1,'he',function(card){
 ...
                return 8-get.value(card)
            ...
            return -1;
        });}else player.chooseControl('<span class=yellowtext>强制命中'+'</span>','cancel2').set('prompt',get.prompt('huokongld')).set('prompt2','令本次攻击命中对手,<br>零级，对面摸两张牌，一级，你弃置一张牌，对面摸一张牌；二级：你弃置一张牌。').set('ai',function(card){
            var evt=_status.event.getTrigger();
            if(get.attitude(evt.player,evt.target)<0){
                if(evt.baseDamage+evt.extraDamage>=Math.min(2,evt.target.hp)){
                    return 1.1
 ...
            event.target=result.targets;//前面有目标target，可以返回target。
    ...
    player.logSkill('fangkong2',event.target);}//让技能发语音，发历史记录。
        ...
        translate:{
            ...
            zhuangjiafh:"装甲防护",
            "zhuangjiafh_info":"拥有护甲时，减少等同于护甲的手牌上限。<br>每当你结算完回复的血量，或受到的伤害时,<br>你可以弃置至多三张红色牌，获得X点护甲。（X为弃牌数）<br>若此时没有红色牌且护甲值没有发生变化，<br>你在伤害结算后获得1点护甲，而无需弃牌。",
            hangmucv:"航母",
            "hangmucv_info":"一轮游戏开始时，你可以弃置两张手牌，<br>视为对非友方角色使用万箭齐发。<br>没有手牌会结束此技能。",
            yuanhang:"远航",
            "yuanhang_info":"受伤时手牌上限+1<br>限x次，当你失去手牌后，且手牌数小于手牌上限/2+1时，你摸一张牌。可在强化中提升X值。<br>当你进入濒死状态时，你摸可以一张牌并获得与舰种对应的技能，体力上限大于二时需减少一点体力上限，额外摸一张牌；死亡后，你可以令一名角色摸三张牌。<br>主公技，开局获得一点护盾",
            qianting:"潜艇",
            "qianting_info":"其他角色计算与你的攻击距离+1,你的攻击距离-1,<br>你使用锦囊牌可以多指定x个目标数（x技能强化的次数），并可以对距离你2以内的角色使用。<br>摸牌阶段，你可选择至多1名角色，你少摸2张牌,然后<br>你对目标使用【顺手牵羊】（目标区域内所有牌的数目大于你时），否则<br>你对目标使用【过河拆桥】。<br>(需要摸牌次数>1);强化技能后：1级:少摸的牌-1；2级:可选择目标+1。<br>一轮游戏开始时，你可以弃置一张牌，对一个目标使用一张雷杀。没有手牌会结束此技能。",
            zhanliebb:"战列",
            "zhanliebb_info":"",
            qingxuncl:"轻巡",
            "qingxuncl_info":"",
            ...
            touxichuan:"袭击",
            "touxichuan_info":"每回合限两次，将♦牌当做火攻，♥牌当做乐不思蜀，♣牌当作过河拆桥，♠牌当做兵粮寸断使用<br>，并为对手施加进水debuff。<br>火攻只需弃置一张牌，其他视为牌需要一张装备牌或者两张基本牌。",
            wulidebuff:"雷火永续",
            ...
            qianghuazhuang:"强化装备",
            "qianghuazhuang_info":"你可以消耗经验，或弃置二至四张牌，选择一个永久效果升级。<br>（如摸牌、攻击距离、手牌上限等）每回合限两次。装备牌代表两张牌",
            huokongld:"火控雷达",
            ...
            "fangkong2":"防空",
            "fangkong2_info":"X为选择的角色数，当一名角色使用的锦囊牌指定了至少两名角色为目标时，<br>你可以弃置X的手牌，令此牌对X名角色无效。<br>(由于机制问题，主动技能完整的转为被动技能比较麻烦))",
            chongsheng1:"重生",
            chongsheng1_info:"濒死阶段，你可将体力回复至2-X，并摸2-X张牌，X为你本局发动此技能的次数。每局最多发动2次",
        },
			};
