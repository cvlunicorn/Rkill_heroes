...
const cangqinghuanying = {
    ...
    chuanchengzhiwu: {//【传承之物】：锁定技。每当其他角色回复体力，你摸一张牌
        audio: 2,
        trigger: { global: 'recoverAfter' },//触发器？监听范围全场。触发时机角色已经回复体力后
        forced: true,//锁定技
        filter: function (event, player) {
            return event.player != player; // 排除自己回复体力的情况
        },
        content: function () {
    ...
    miaoyi: {//喵噫！】：每当你使用"闪"抵消一张"杀"，你可以立即对该角色使用一张手牌当"杀"。此效果每轮限一次。
        trigger: {//使用闪触发。时机
            player: 'respondAfter' //打出，因为只是响应杀。用闪响应杀这个时机触发
        },
        round: 1,//每轮限一次
        ...
        content() {
            'step 0'//第一步
            var source = trigger.getParent(2).player; // 获取出杀的角色
            player.chooseCard('h', '是否将一张手牌当【射击】对' +//h手牌
                get.translation(source) + '使用？')
                .set('ai', function (card) {//关于判断。不打队友，不用7以上价值的牌，优先使用低权重牌
                    var att = get.attitude(player, source);//计算态度
                    if (att > 0)  // 如果是队友，不打
                        return 0;

                    return 7 - get.value(card); // 价值越低权重越高。获取卡牌价值，反转。只要7权重以下的，所以不需要再筛选高权重了

                });
            //第一步是锁定目标。
            'step 1'//第二步应该是执行杀？
            if (result.bool) {//如果点了确认
                player.useCard(
                    { name: 'sheji9' },//视为的卡牌
                    result.cards,//实际消耗的卡牌
                    source, //单目标
                    false//不计数
                );//射击
                //没写不能无懈可击所以就是可以无懈可击
